...
| **`core`** | [`agents/core-agent.md`](./agents/core-agent.md) | `:core` | Mantém os tokens de design (Cores primitivas e semânticas, Espaçamentos, Raios, Tamanhos, Elevações e Animações). |
| **`components`** | [`agents/components-agent.md`](./agents/components-agent.md) | `:components` | Desenvolve átomos e moléculas reutilizáveis em Jetpack Compose com State Hoisting, acessibilidade e screenshot tests. |
| **`templates`** | [`agents/templates-agent.md`](./agents/templates-agent.md) | `:templates` | Desenvolve telas completas e fluxos desacoplados baseados em `UiState` + `BaseViewModel` + `FakeViewModel`. |
