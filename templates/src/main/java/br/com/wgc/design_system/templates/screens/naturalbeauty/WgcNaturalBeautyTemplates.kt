package br.com.wgc.design_system.templates.screens.naturalbeauty
...
fun WgcNaturaHomeTemplate(modifier: Modifier = Modifier) {
    Scaffold(
        ...
        topBar = {
            Column(modifier = Modifier.fillMaxWidth().background(Color(WgcCoreDsColors.naturalBeautyOrange)).padding(WgcCoreDsSpacing.md16.dp)) {
                Text("Beleza Natural • Bem Estar Bem", fontWeight = FontWeight.Bold, fontSize = 14.sp, color = Color.White)
            }
        }
    ) { padding ->
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(WgcCoreDsSpacing.md16.dp), verticalArrangement = Arrangement.spacedBy(WgcCoreDsSpacing.sm12.dp)) {
            items(NaturalBeautyMockData.sampleProducts) { prod ->
...
fun WgcNaturaConsultantTemplate(modifier: Modifier = Modifier) =
    WgcGenericPlaceholderTemplate(title = "Espaço da Consultora", modifier = modifier)
...
fun WgcNaturaBagTemplate(modifier: Modifier = Modifier) =
    WgcGenericPlaceholderTemplate(title = "Sacola de Cosméticos", modifier = modifier)

@Composable
fun WgcNaturaProfileTemplate(modifier: Modifier = Modifier) =
    WgcGenericPlaceholderTemplate(title = "Meu Perfil & Pontos", modifier = modifier)
