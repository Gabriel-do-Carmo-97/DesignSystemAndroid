package br.com.wgc.ds_templates.screens.channelmessaging
...
fun WgcTelegramChatsTemplate(modifier: Modifier = Modifier) {
    Scaffold(
        ...
        topBar = {
            Column(modifier = Modifier.fillMaxWidth().background(Color(WgcCoreDsColors.channelMessagingBlue)).padding(WgcCoreDsSpacing.md16.dp)) {
                Text("Canais de Mensagem", fontWeight = FontWeight.Bold, fontSize = 14.sp, color = Color.White)
            }
        }
    ) { padding ->
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(WgcCoreDsSpacing.md16.dp), verticalArrangement = Arrangement.spacedBy(WgcCoreDsSpacing.sm12.dp)) {
            items(ChannelMessagingMockData.sampleChannels) { ch ->
...
fun WgcTelegramSettingsTemplate(modifier: Modifier = Modifier) =
    WgcGenericPlaceholderTemplate(title = "Configurações de Canais", modifier = modifier)
