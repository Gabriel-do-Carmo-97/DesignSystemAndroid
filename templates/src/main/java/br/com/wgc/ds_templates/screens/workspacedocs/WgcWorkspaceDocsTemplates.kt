package br.com.wgc.ds_templates.screens.workspacedocs
...
fun WgcNotionWorkspaceTemplate(modifier: Modifier = Modifier) {
    Scaffold(
        ...
        topBar = {
            Column(modifier = Modifier.fillMaxWidth().background(Color(WgcCoreDsColors.workspaceDocsBlack)).padding(WgcCoreDsSpacing.md16.dp)) {
                Text("Workspace Docs • Produtividade", fontWeight = FontWeight.Bold, fontSize = 14.sp, color = Color.White)
            }
        }
    ) { padding ->
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding), contentPadding = PaddingValues(WgcCoreDsSpacing.md16.dp), verticalArrangement = Arrangement.spacedBy(WgcCoreDsSpacing.sm12.dp)) {
            items(WorkspaceDocsMockData.sampleDocs) { doc ->
...
fun WgcNotionProfileTemplate(modifier: Modifier = Modifier) =
    WgcGenericPlaceholderTemplate(title = "Perfil do Workspace & Times", modifier = modifier)
